    ...
    public void DrawRushBar(Graphics g, Player player, bool isRPressed)
{
    ...
    // Вводим фактор времени для анимаций (синусоида от текущего времени)
    ...
    if (isReady)
    {
        // Симулируем резкое дерганье кадра в стиле Hotline Miami
        var rand = new Random();
    ...
    // 1. ЗАДНИЙ ФОН (Темно-фиолетовый неоновый провал вместо скучного серого)
    ...
    // 2. ЗАПОЛНЕНИЕ ШКАЛЫ (Неоновые цвета Hotline Miami)
    ...
    if (currentFillWidth > 0)
    {
        ...
        if (isReady)
        {
            if (isRPressed)
            {
                // Если клавиша R зажата и заряд полон — шкала яростно мигает белым и розовым
                barColor = (int)(time * 15) % 2 == 0 ? Color.White : Color.FromArgb(255, 0, 128);
            }
            else
            {
                // Заряд готов: плавно переливается из неонового бирюзового (Cyan) в ядовито-розовый (Hot Pink)
    ...
    // 3. НЕОНОВАЯ ОБВОДКА (Яркий контрастный контур)
    // Если готов — обводка розовая, если нет — тускло-фиолетовая
    Color borderColor = isReady ? Color.FromArgb(255, 0, 128) : Color.FromArgb(90, 30, 120);
    using (Pen borderPen = new Pen(borderColor, 3)) // Сделали рамку толще для комиксного стиля
    {
        g.DrawRectangle(borderPen, barX, barY, barWidth, barHeight);
    }

    // 4. ТЕКСТ В СТИЛЕ 80-х (Сдвинут вправо, использует курсив и яркий цвет)
    // Текст тоже немного дергается при зажатом R
    int textX = barX + barWidth + 12 + (isRPressed ? new Random().Next(-1, 2) : 0);
    int textY = barY - 2;

    using (Font font = new Font("Impact", 11, FontStyle.Italic)) // Шрифт Impact дает агрессивный ретро-вид
    {
        ...
        // Рисуем черную тень для текста (важный элемент стиля VHS/Ретро)
